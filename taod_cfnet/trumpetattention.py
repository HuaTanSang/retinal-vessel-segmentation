...
class TrumpetAttention(nn.Module):
    def __init__(self, in_channels, threshold=0.9, coef=0.1):
        ...
        self.doubleconv_horizon = DoubleConv(in_channels, in_channels)  # Cho nhánh Horizontal
        self.doubleconv_vertical = DoubleConv(in_channels, in_channels)  # Cho nhánh Vertical
    ...
    def forward(self, x):  # x: (B, C, H, W)
        ...
        x_horizon_21 = self.doubleconv_horizon(x_horizon_2)
        x_horizon_21 = F.interpolate(x_horizon_21, size=(H, W), mode='bilinear', align_corners=False)
        ...
        x_vertical_21 = F.interpolate(x_vertical_21, size=(H, W), mode='bilinear', align_corners=False)
